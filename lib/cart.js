...
module.exports = function (restClient) {
    ...
    module.create = function (customerToken, customerId = null, headers) {
        if (customerId) {
            return restClient.post('/customers/' + customerId + '/carts', {}, customerToken, headers);
        } else {
            if (customerToken) {
                return restClient.post('/carts/mine', {}, customerToken, headers);
            } else 
            {
    ...
    module.update = function (customerToken, cartId, cartItem, adminRequest = false, headers) {
        if (adminRequest) {
            return restClient.post('/carts/' + cartId + '/items/', { cartItem: cartItem }, customerToken, headers);
        } else {
            if (customerToken && isNumeric(cartId)) {
                return restClient.post('/carts/mine/items', { cartItem: cartItem }, customerToken, headers);
            } else 
            {
    ...
    module.shippingInformation = function (customerToken, cartId, body, adminRequest = false, headers) {
        if (adminRequest) {
            return restClient.post('/carts/' + cartId + '/shipping-information', body, customerToken, headers);
        } else {
            if (customerToken && isNumeric(cartId)) {
                return restClient.post('/carts/mine/shipping-information', body, customerToken, headers);
            } else 
            {
    ...
    module.order = function (customerToken, cartId, body, adminRequest = false, headers) {
        if (adminRequest) {
            return restClient.put('/carts/' + cartId + '/order', body, customerToken, headers);
        } else {
            if (customerToken && isNumeric(cartId)) {
                return restClient.put('/carts/mine/order', body, customerToken, headers);
            } else 
            {
